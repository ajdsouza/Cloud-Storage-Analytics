#!/usr/local/bin/perl
# 
# $Header: storage_report_metrics.pl 21-jul-2004.17:14:38 ajdsouza Exp $
#
...
#
#    MODIFIED   (MM/DD/YY)
#    ajdsouza    07/19/04 - Use the agentstatedir for caching, pretty indentation
...
BEGIN
  {
    ...
    $agent_state_dir = $ENV{EM_AGENT_STATE} if $ENV{EM_AGENT_STATE};

    stat($agent_state_dir) if $agent_state_dir;

    $agent_state_dir = Cwd::abs_path() unless $agent_state_dir and -e $agent_state_dir and -d $agent_state_dir;

    # Create the storage directory if one does not exit
    $agent_state_dir = catfile($agent_state_dir,'storage');

    stat($agent_state_dir);
    
    mkpath($agent_state_dir,0,0777) unless -e $agent_state_dir;

    stat($agent_state_dir);

    $agent_state_dir = Cwd::abs_path() unless $agent_state_dir and -e $agent_state_dir and -d $agent_state_dir and -w $agent_state_dir;

    my $logfile = catfile($agent_state_dir,'nmhstrc.log');
    ...
    open(STDERR,"> $logfile") or open(STDERR,"> $devnull");
...
$storage_columns{issues} = { 1=>'type', 2=>'message', 3=>'message_nls_id', 4=>'message_params', 5=>'action_nls_id', 6=>'action_params' };
...
my @error_stack;
$SIG{'__DIE__'} = sub {  push @error_stack, "ERROR : $_[0] "; exit_fail()};
$SIG{'__WARN__'} = sub { push @error_stack,"$_[0]"; warn "$_[0]"; };
...
sub dump_raw_metrics_to_file ( $\@ )
  {
    ...
    my $raw_metrics_file = catfile($agent_state_dir,$file_name);
    
    stat($raw_metrics_file);
    
    open(FH,'>',$raw_metrics_file) or warn "Failed to open the file $raw_metrics_file to dump raw metrics while generating storage metrics\n" and return;
    ...
    close(FH) or warn "Failed to close the file $raw_metrics_file while generating metrics \n" and return 1;
	...
	for my $key_value ( sort {$index_entity_key_value{$a}->{global_unique_id} cmp $index_entity_key_value{$b}->{global_unique_id}}  keys %{$node->{child_node_list}} )
	  {
	    ...
	    warn "Global unique ID is null for $key_value while generating gid for $node->{key_value}\n" and return unless $index_entity_key_value{$key_value}->{global_unique_id};
...
sub collect_raw_storage_metrics( \% ) 
  {
	...
	$array_of_metrics_hash_refs_from_function = $list_of_storage_metric_functions{$function_order}->() or warn "Failed to execute function at $function_order\n" and return;
	
	mark_parent_key_values_based_on_key_criteria( @$array_of_metrics_hash_refs_from_function ) or warn "Failed to mark the parent_key_values for metric $function_order\n" and return;
    ...
    dump_raw_metrics_to_file 'nmhsrmet.log',@all_metric_data or return;
	...
	warn "key_value cannot be null in key map\n" and return unless $metric_hash_data_ref->{key_value};
...
sub generate_parent_child_relationship_for_os_visible_storage_entities( \% ) 
  {
	...
	for my $key_value( keys %{$index_entities_with_os_path{os_identifier}{$os_path}} )
	  {
	    ...
	    my $os_storage_entity_name = storage::Register::get_os_storage_entity_path($os_path) or warn "Failed to get the OS storage entity name $os_path\n" and return;
	    
	    my $entity_identifier =  storage::Register::get_os_identifier_for_os_path ($os_storage_entity_name) or warn "Failed to get the os identifier for storage entity name $os_storage_entity_name\n" and return;
	...
	warn "Failed to find an identifier on the OS for $index_entity_key_value{$key_value}->{os_identifier}\n"  and return unless @{$index_entity_key_value{$key_value}->{entity_identifier}};
	...
	for my $entity_identifier ( @{$index_entity_key_value{$key_value}->{entity_identifier}} )
	  {
		...
		warn "No hierarchy defined for layers\n" and next unless $storage_layer_hierarchy{$index_entity_key_value{$key_value}->{storage_layer}} and $storage_layer_hierarchy{$os_id_storage_data_ref->{storage_layer}};
...
sub collate_and_build_the_keys_table_for_all_entities(\%)
  {
	...
	# ERROR If there is NO entry in key master for node
	warn "key_value cannot be null in key map\n" and return unless $child_key_value;
	...
	for my $parent_key_value ( @{$index_entity_key_value{$child_key_value}->{parent_keys}} )
	  {
	    ...
	    warn " Unable to find the entry in key master for parent node $parent_key_value\n" and return unless $parent_key_value and defined $index_entity_key_value{$parent_key_value};
...
sub process_collected_storage_metrics(\%) 
  {
    ...
    traverse_tree_after_processing_node \%top_nodes ,'child_node_list',&np_mark_node_indent or warn "Failed to mark child tree indent\n" and return;
    traverse_tree_after_processing_node \%bottom_nodes ,'parent_node_list',&np_mark_node_indent or warn "Failed to mark parent tree indent\n" and return;
    traverse_tree_before_processing_node \%top_nodes ,'child_node_list',&np_generate_global_unique_id or warn "Failed to generate the global unique_id for all nodes in the storage tree\n" and return;
    traverse_tree_before_processing_node \%top_nodes ,'child_node_list',&np_generate_query_flag or warn "Failed to generate the global unique_id for all nodes in the storage tree\n" and return;
...
sub cache_storage_metrics_to_file(\%)
  {
	...
	my $metric_file = catfile($agent_state_dir,'nmhs'.substr($metric_name,0,4).'.txt');
	
	stat($metric_file);
	
	open(FH,'>',$metric_file) or close(FH) and warn "Failed to open the mapfile $metric_file while generating metrics\n" and return;
	...
	close(FH) or warn "Failed to close the file $metric_file while generating metrics \n" and return;
...
sub display_metrics_from_file($)
  {
    ...
    open(FH,"$metric_file") or warn "Failed to open the cached file $metric_file for $metric_name while reading metrics\n" and return;
...
warn "Unsupported storage_report metric $metric_name\n" and exit_fail() unless $metric_name =~ /^(data|keys|issues|alias)$/;
...
( generate_storage_metrics or warn " Failed to generate storage metrics for $metric_name\n" and exit_fail() ) if $metric_name =~ /^data$/ or not -e "$metric_file";

display_metrics_from_file($metric_name) or warn "Failed to read storage metrics for $metric_name from file\n" and exit_fail();
