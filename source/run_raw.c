...
*
*  $Id: run_raw.c,v 1.3 2003/03/11 01:25:58 ajdsouza Exp $ 
*
*
* NAME  
*	 run_raw.c
*
* DESC 
*	execute raw to list the veritas volumes on a host
*	The binary requires suid to be set to execute raw
*
*
* FUNCTIONS
*
...
#define BUFFER_SIZE	256		/* Max size of command line arguments */
	...
	int main (int argc , char **argv)
	{
		...
		if ( argc < 2 )
		{
		  printf("error::Usage : run_raw <args>\n");
		  exit(EXIT_FAILURE);
		}
		...
		if (!path )
		{
		  printf("error::run_raw.c :  raw not found \n");
		  exit(EXIT_FAILURE);
		}
		...
		if ( !args )
		{
		  printf("error::run_raw.c: Failed to allocated buffer to arg \n");
		  exit(EXIT_FAILURE);
		}

		args[0] =  (char *)calloc(strlen("raw") + 1,sizeof(char));
		if ( !args[0] )
		{
		  printf("error::run_raw.c: Failed to allocated buffer to args[0] \n");
		  exit(EXIT_FAILURE);
		}
		...
		strcpy(args[0],"raw");
   		...
   		for (i=1; i<argc; i++)
   		{
			...
			{
			  printf("error::run_raw.c: Arg > than buffer size %d \n",BUFFER_SIZE);
			  exit(EXIT_FAILURE);
			}
			...
			for(j=0; j<size;j++)
			{
				...
				if ( strlen(args[i]) != 2)  
				{				 
				  printf("error::run_raw.c : Invalid arg length %s\n",args[i]);
				  exit(EXIT_FAILURE);
				}
				...
				if ( j == size-1 )
				{				  
				  printf("error::run_raw.c : Invalid argument %s \n",args[i]);
				  exit(EXIT_FAILURE);
				}
